...
//
Функция Что(Знач ПроверяемоеЗначение, Знач Сообщение = "", ТекстПроверяемоеЗначение = Неопределено) Экспорт
	...
	Если ТекстПроверяемоеЗначение <> Неопределено Тогда
		Контекст.ОбъектПроверки.ТекстПроверяемоеЗначение = ТекстПроверяемоеЗначение;
	КонецЕсли;
...
Процедура СгенерироватьОшибкуСравнения(Сообщение, ФактическоеЗначение, ОжидаемоеЗначение, ОбъектПроверки = "проверяемое значение")
	...
	Контекст = Контекст();
	ЮТРегистрацияОшибок.СгенерироватьОшибкуСравнения(Контекст, Сообщение, ФактическоеЗначение, ОжидаемоеЗначение, Контекст.ОбъектПроверки.ТекстПроверяемоеЗначение);
...
Процедура ПроверитьПредикат(Выражение, ОжидаемоеЗначение, ОписаниеПроверки, ПараметрыСравнения = Неопределено)
	...
	Результат = ЮТПредикатыКлиентСервер.ПроверитьПредикаты(Контекст.ПроверяемоеЗначение, Предикат, ПараметрыСообщенийОбОшибке, ПараметрыСравнения);
